...
def preprocess_true_boxes(bboxes, train_output_sizes, strides, num_classes, max_bbox_per_scale, anchors):
    label = np.zeros((train_output_sizes[0][0], train_output_sizes[1][0], 3,
                       5 + num_classes))
    bboxes_xywh = np.zeros((max_bbox_per_scale, 4))
    bbox_count = np.zeros((3,))
    for bbox in bboxes:
        bbox_coor = bbox[:4]
        #print("bbox_class_ind",bbox[4])
        ...
        # 防止越界
        grid_r = label.shape[0]
        grid_c = label.shape[1]
        xind = max(0, xind)
        yind = max(0, yind)
        ...
        label[yind, xind, best_anchor, 5:] = onehot
        bbox_ind = int(bbox_count[0] % max_bbox_per_scale)
...
def multi_thread_read(batch, num, train_input_size, annotation_type, train_output_sizes, strides, num_classes, max_bbox_per_scale, anchors, batch_image,
    ...
    label_sbbox, sbboxes = preprocess_true_boxes(bboxes, train_output_sizes, strides, num_classes, max_bbox_per_scale, anchors)
...
def generate_one_batch(annotation_lines, step, batch_size, anchors, num_classes, max_bbox_per_scale, annotation_type):
    ...
    return batch_image, [batch_label_sbbox, batch_sbboxes]
...
if __name__ == '__main__':
    ...
    if pattern == 2:
        ...
        initial_epoch = 0
        epochs = 100
        optimizer = torch.optim.Adam(net.parameters(), lr=lr,weight_decay=0.0001)  # 传入 net 的所有参数, 学习率
    ...
    for t in range(initial_epoch, epochs, 1):
        ...
        for step in range(train_steps):
            ...
            y3_pred = net(batch_image)  # 直接卷积后的输出
            args = [y3_pred] + lables
