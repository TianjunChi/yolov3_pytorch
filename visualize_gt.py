...
LastEditTime: 2023-09-26 19:20:21
'''
import cv2
import numpy as np
import torch
...
def draw(anno_dir,frame_num):
    ...
    for size in sizes:
        ...
        if success == False:
            ...
            # cv2.imwrite('single_head_output_320_160/single_head_'+str(frame_num)+'.png',gt_image+pre_image)
            # cv2.imshow('pre',pre_image)
        """
        #cv2.imshow('gt',gt_image)
        cv2.imwrite('gt_bbox/'+str(frame_num)+'.png',gt_image)#+pre_image)
        #cv2.waitKey(2000)  # Wait for 1 second (1000 milliseconds)
...
if __name__ == '__main__':
    ...
    gt_path = "E:\yolov3_pytorch\\annotation\data_train.txt" 
    ...
    for i in range(0,2506):#506
        draw(gt_path,i) # i 代表按照val.txt中的顺序（非顺序排列）的第i张图片 
