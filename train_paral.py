...
def train(net,num_train,num_val,yolo_loss,batch_size,epochs,initial_epoch,train_lines,val_lines,save_best_only,anchors, num_classes,max_bbox_per_scale,pattern,lr,optimizer,checkpoint):
...
if __name__ == '__main__':
    ...
    if pattern == 2: # load yolo coco pretrained model
        lr = 0.0001
        batch_size = 8
        initial_epoch = 0
        epochs = 999
    ...
    elif pattern == 1: # load pretrained model 
        #lr = 0.000001
        lr = 0.0001
        batch_size = 6
        initial_epoch = 100
    ...
    for rank in range(num_processes):
        p = mp.Process(target=train, args=(net_img,num_train,num_val,yolo_loss,batch_size,epochs,initial_epoch,train_lines,val_lines,save_best_only,anchors, num_classes,max_bbox_per_scale,pattern,lr,optimizer,checkpoint))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()