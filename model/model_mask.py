...
class MaskUnit(torch.nn.Module): # Mask + BN + ReLu
    def __init__(self, input_dim, filters, kernels, stride, padding):
        super(Conv2dUnit, self).__init__()
        self.conv = torch.nn.Conv2d(input_dim, filters, kernel_size=kernels, stride=stride, padding=padding, bias=False)
...
# Define a custom mask layer for element-wise multiplication
class MaskLayer(torch.nn.Module):
    def __init__(self, input_size):
        super(MaskLayer, self).__init__()
        self.mask = torch.nn.Parameter(torch.ones((input_size,8,8)), requires_grad=True)
...
class Darknet(torch.nn.Module):
    def __init__(self, num_classes, initial_filters=32):
        ...
        ''' darknet53部分结束 '''
        # add a mask layer after feature extraction
        self.mask1 = MaskLayer(i1024)
        self.mask2 = MaskLayer(i512)
        self.mask3 = MaskLayer(i256)
        # print(i1024)
...
if __name__ == '__main__':
        """
        # 验证点乘
        x1_ = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
        x1 = torch.tensor(x1_, dtype=torch.float32)

        x2_ = [[1, 1, 1], [0, 0, 0], [1, 1, 1]]
        x2 = torch.tensor(x2_, dtype=torch.float32)
        res = x1*x2
        print(res)
        """
        # 验证darknet53和mask
