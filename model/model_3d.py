...
# Example Conv3d and Conv2d layers
conv3d_layer = torch.nn.Conv3d(in_channels=16, out_channels=33, kernel_size=3, stride=2)
conv2d_layer = torch.nn.Conv2d(in_channels=33, out_channels=64, kernel_size=3, stride=1)

# Create a random 5D input tensor (batch_size, channels, depth, height, width)
input_tensor = torch.randn(20, 16, 10, 50, 100)

# Apply the Conv3d layer to the input tensor
conv3d_output = conv3d_layer(input_tensor)

# Get the dimensions of the Conv3d output
batch_size, channels, depth, height, width = conv3d_output.size()

# Reshape the Conv3d output to be a 2D tensor
conv3d_output_reshaped = conv3d_output.view(batch_size, channels * depth, height, width)

# Now you can use the reshaped tensor as input to the Conv2d layer
conv2d_output = conv2d_layer(conv3d_output_reshaped)

# Check the shape of the Conv2d output
print(conv2d_output.shape)

class Conv3dUnit(torch.nn.Module): # Conv + BN + ReLu
    def __init__(self, input_dim, filters, kernels, stride, padding):
        super(Conv3dUnit, self).__init__()
        self.conv = torch.nn.Conv3d(input_dim, filters, kernel_size=kernels, stride=stride, padding=padding, bias=False)
        self.bn = torch.nn.BatchNorm3d(filters)
        self.leakyreLU = torch.nn.LeakyReLU(0.1)
...
class Darknet(torch.nn.Module):
    def __init__(self, num_classes, initial_filters=32):
        ...
        self.conv3_0 = Conv3dUnit(3, i16, (3,3,3),stride=1,padding=1)
        self.conv3_1 = Conv3dUnit(i16, i32, (3,3,3),stride=2,padding=1)
        self.conv1 = Conv2dUnit(i16, i32, (3, 3), stride=1, padding=1)
